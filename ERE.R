...
es <- bugs(data=data, inits=NULL,parameters.to.save=params,model.file=filein,
...
e[,1] <- -l*p1
e[,2] <- -l*p2
muce <- function(a,b) {     #a:p1,b:p2
  mu.c1 <- a*unit.cost.comp*mu.l.comp+(1-a)*unit.cost.no*mu.l.no
  mu.c2 <- c.anti+b*unit.cost.comp*mu.l.comp+(1-b)*unit.cost.no*mu.l.no
  mu.e1 <- a*(365-mu.l.comp+mu.l.comp*unit.e.comp)+(1-a)*(365-mu.l.no+mu.l.no*unit.e.no)
  mu.e2 <- b*(365-mu.l.comp+mu.l.comp*unit.e.comp)+(1-b)*(365-mu.l.no+mu.l.no*unit.e.no)
}
NBt <- function(a,b,k) {
